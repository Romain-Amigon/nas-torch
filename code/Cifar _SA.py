...
if __name__ == "__main__":
    ...
    for current_seed in seeds:
        ...
        opt_abc = SAOptimizer(layers=initial_arch , dataset=train_loader_proxy,)
        opt_abc.evaluate = types.MethodType(evaluate_cifar_proxy, opt_abc)
        best_sol_final, optim_stats_abc = opt_abc.run(100)
    ...
    with open("results/academic_results_SA_100.json", "w", encoding="utf-8") as f:
        json.dump(all_results, f, indent=4)

    with open("results/academic_summary_SA_100.txt", "w", encoding="utf-8") as f:
