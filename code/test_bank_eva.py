...
# --- Fonction d'évaluation sur-mesure pour données déséquilibrées ---
def evaluate_f1(self, genome, train_epochs=5):
    try:
        model = DynamicNet(genome, input_shape=(29,))
        model.to(DEVICE)
        ...
        with torch.no_grad():
            for inputs, targets in val_loader:
                ...
                # On utilise un seuil strict dès la phase de recherche
                preds = (probs > 0.95).float().cpu().numpy()
                all_preds.extend(preds)
                all_targets.extend(targets.numpy())

        # On renvoie le F1-Score (multiplié par 100 pour garder l'échelle habituelle du NAS)
        score = f1_score(all_targets, all_preds) * 100.0
        return score
    except Exception:
        return -float('inf')
...
import types
opt_trans.evaluate = types.MethodType(evaluate_f1, opt_trans)

print("Début de la recherche Transformer (Optimisation F1-Score)...")
best_arch_trans, stats_trans = opt_trans.run(20) # Augmenté à 20 pour laisser le temps de chercher

opt_abc = ABCOptimizer(layers=best_arch_trans, dataset=train_loader, limit=5, patience=5)
opt_abc.evaluate = types.MethodType(evaluate_f1, opt_abc)
