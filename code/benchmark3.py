...
from sklearn.datasets import fetch_covtype, make_friedman1
...
BATCH_SIZE = 64
N_SAMPLES_TRAIN_IMG = 4000
N_SAMPLES_TEST_IMG = 1000
N_STATS_RUNS = 5
ITERATIONS_OPTIM = 50

def get_dataset(task_type):
    if task_type == 'friedman_regression':
        X, y = make_friedman1(n_samples=20000, n_features=20, noise=0.5, random_state=42)
        ...
        return train_loader, test_loader, (1, 20), 1

    elif task_type == 'covertype_classification':
        data = fetch_covtype()
        ...
        train_loader = DataLoader(TensorDataset(X_train, y_train), batch_size=BATCH_SIZE, shuffle=True)
        test_loader = DataLoader(TensorDataset(X_test, y_test), batch_size=BATCH_SIZE)
        return train_loader, test_loader, (1, 54), 7

    elif 'cifar10' in task_type:
...
def get_initial_arch(task_type, input_shape, output_dim):
    ...
    if 'resblock' in task_type:
        ...
        layers.append(FlattenCfg()) 
        layers.append(LinearCfg(in_features=0, out_features=64, activation=nn.ReLU))
        layers.append(BatchNorm1dCfg(num_features=0))
        layers.append(DropoutCfg(p=0.2))
...
if __name__ == "__main__":
    tasks = ["friedman_regression", "covertype_classification", "cifar10_simple", "cifar10_resblock"]
    ...
    print("\n" + "="*130)
    print(f"COMPLEX BENCHMARK SUITE (Runs: {N_STATS_RUNS} | Iterations/Gen: {ITERATIONS_OPTIM})")
    print("="*130)
